...
%%
program:	 component_list { treeRoot = $1; }
		 ;

function_preamble: ftype declared_func_identifier '('	{
															$$ = newTreeNode();
															$$->type = TN_FDEF;
															$$->sval = strdup($2);
															PushTable(symStack);
...
function_header: function_preamble parameter_list ')'	{
																			$$ = $1;
																			$$->numOperands = 1;
																			$$->operands[0] = $2;
																		}
...
function:	function_header statement_list END	{
													$$ = newTreeNode();
													$$->type = TN_FUNCTION;
													$$->numOperands = 2;
													$$->operands[0] = $1;
...
import_statement:	IMPORT STRING_LITERAL ';' { free($2); }

component_list:	function
				| function component_list { $1->pNextStatement = $2; $$ = $1; }
				| import_statement component_list { $$ = $2; }
				;

statement: decl_assign_statement
...
statement_list:	statement
				| decl_statement statement_list { $$ = $2; }
...
decl_assign_statement: BYTE IDENT_UNDEC '=' int_exp ';'		{
...
decl_statement: BYTE IDENT_UNDEC ';' { AddSymbol(symStack, $2, IT_BYTE); free($2); }
				| WORD IDENT_UNDEC ';' { AddSymbol(symStack, $2, IT_WORD); free($2); }
				| BOOL IDENT_UNDEC ';' { AddSymbol(symStack, $2, IT_BOOL); free($2); }
				| BYTEP IDENT_UNDEC ';' { AddSymbol(symStack, $2, IT_BYTEP); free($2); }
				| WORDP IDENT_UNDEC ';' { AddSymbol(symStack, $2, IT_WORDP); free($2); }
				| vtype declared_identifier ';' { yyerror("symbol already declared"); free($2); }
				;

assignment_statement:	IDENT_BYTE '=' int_exp ';'		{
															$$ = newTreeNode();
															$$->type = TN_BYTE_ASSIGN;
															struct tree_node* ident = newTreeNode();
															ident->type = TN_BYTE_IDENT;
...
declared_identifier: IDENT_BYTE
					 | IDENT_WORD
					 | IDENT_BOOL
					 | IDENT_BYTEP
					 | IDENT_WORDP
...
declared_func_identifier:	FIDENT_VOID
							| FIDENT_BYTE
							| FIDENT_WORD
							| FIDENT_BOOL
							| FIDENT_BYTEP
...
while_header: WHILE '(' bool_exp ')'	{
											PushTable(symStack);
											$$ = $3;
										}
			  ;

while_statement: while_header statement_list END	{
														PopTable(symStack);
														$$ = newTreeNode();
														$$->type = TN_WHILE;
														$$->numOperands = 2;
...
if_header: IF '(' expression ')' { PushTable(symStack); $$ = $3; }
		   ;

else_clause: ELSE statement_list END { $$ = $2; }
			 ;

elseif_header: ELSEIF '(' expression ')' { PushTable(symStack); $$ = $3; }
			   ;

elseif_block: elseif_header statement_list END	{
													PopTable(symStack);
													$$ = newTreeNode();
													$$->type = TN_IF;
													$$->numOperands = 2;
...
if_statement: if_header statement_list END	{
												PopTable(symStack);
												$$ = newTreeNode();
												$$->type = TN_IF;
												$$->numOperands = 2;
...
asm_statement: ASM asm_list END	{
									$$ = newTreeNode();
									$$->type = TN_ASM;
									$$->numOperands = 1;
									$$->operands[0] = $2;
...
asm_memloc: '[' declared_identifier ']' {
											$$ = newTreeNode();
											$$->type = TN_ASMLOC;
											$$->numOperands = 1;
											$$->operands[0] = newTreeNode();
...
ptr_function_call:	FIDENT_BYTEP '(' ')'	{
												$$ = newTreeNode();
												$$->type = TN_FUNCTIONCALL;
												$$->sval = strdup($1);
...
void_function_call:	FIDENT_VOID '(' ')'	{
											$$ = newTreeNode();
											$$->type = TN_FUNCTIONCALL;
											$$->sval = strdup($1);
...
undec_function_call:	IDENT_UNDEC '(' ')'	{
												$$ = newTreeNode();
												char err[500];
...
return_statement:	RETURN int_exp ';'	{
											$$ = newTreeNode();
											$$->type = TN_RET_INT;
											$$->numOperands = 1;
											$$->operands[0] = $2;
